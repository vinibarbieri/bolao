...
function LoginForm() {
  ...
  const [email, setEmail] = useState(isDev ? "a@a.com" : "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === "signup") {
      ...
      // Confirm-email is OFF, so a real new signup returns a session.
      // If there's no session, Supabase obfuscated an existing email —
      // nudge the user to log in instead.
      if (!data.session) {
        setError(t("signupNoSession"));
        setMode("login");
        setLoading(false);
        return;
      }
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background p-4">
      ...
      <Card className="relative w-full max-w-md shadow-lg">
        ...
        <CardContent className="space-y-4">
          ...
          {isDev && (
            <p className="rounded border bg-muted px-2 py-1 text-center text-xs text-muted-foreground">
              {t("devMode")}
            </p>
          )}
        </CardContent>
